...
task schedule	agent-testbench task schedule NAME --command COMMAND --interval DURATION [--store NAME_OR_DSN] [--shell] [--notify-file PATH] [--notify-webhook URL] [--json]	surface=default	reason=task intent: persists interval schedules for an explicit foreground worker
task worker	agent-testbench task worker [--store NAME_OR_DSN] [--once] [--poll-interval DURATION] [--json]	surface=default	reason=task intent: claims and executes due interval tasks from the active Store
...
task stop	agent-testbench task stop NAME [--store NAME_OR_DSN] [--recover-running-claim --confirm-side-effects-reviewed] [--json]
...
environment restore	agent-testbench environment restore ENV_ID --workspace PATH [--store NAME_OR_DSN] [--execute] [--pull] [--prepare-repos-only] [--assume-clean-docker] [--use-existing-containers] [--clean-docker-state] [--clean-docker-images] [--allow-destructive-docker-cleanup] [--run-workflow --server-url URL] [--health-timeout-seconds N] [--output-format text|json|stream-json] [--json]	surface=default	reason=environment lifecycle: inspect, restore, check, stop, or restart a registered environment
environment status	agent-testbench environment status ENV_ID --workspace PATH [--store NAME_OR_DSN] [--health-timeout-seconds N] [--json]	surface=default	reason=environment lifecycle: inspect, restore, check, stop, or restart a registered environment
...
environment acceptance start	agent-testbench environment acceptance start ENV_ID --server-url URL --request-id ID [--timeout-seconds N] [--json]
...
map import-workflows	agent-testbench map import-workflows [--store NAME_OR_DSN] [--map ID] [--workflow ID] [--append] [--display-name NAME] [--description TEXT] [--json]
...
map validation promote	agent-testbench map validation promote --map ID (--case CASE_ID | --node NODE_ID) [--interface ID] [--store NAME_OR_DSN] [--json]
...
case catalog upsert	agent-testbench case catalog upsert --case ID --node ID [--store NAME_OR_DSN] [--profile ID] [--expected-revision N] [--display-name NAME] [--description TEXT] [--request-template ID] [--case-type TYPE] [--scenario TEXT] [--tag TAG]... [--priority PRIORITY] [--owner OWNER] [--status STATUS] [--sort-order N] [--case-path PATH] [--source-kind KIND] [--source-path PATH] [--executor ID] [--base-url URL] [--evidence-dir PATH] [--timeout-seconds N] [--render-mode MODE] [--patch-json JSON] [--expected-json JSON] [--default-override KEY=VALUE]... [--default-overrides-json JSON] [--json]	surface=default	reason=case maintenance: create and revise Store-backed cases with optimistic concurrency
case catalog history	agent-testbench case catalog history [--store NAME_OR_DSN] [--profile ID] [--limit N] [--json]	surface=default	reason=case maintenance: review immutable catalog revisions without exposing raw configuration secrets
case catalog rollback	agent-testbench case catalog rollback --revision N [--expected-revision N] [--store NAME_OR_DSN] [--profile ID] [--json]	surface=default	reason=case maintenance: restore historical content as a new catalog revision
...
case config upsert	agent-testbench case config upsert --case ID [--workflow ID --step ID] [--store NAME_OR_DSN] [--expected-revision N] [--config-id ID] [--node-id ID] [--status STATUS] [--method METHOD] [--path PATH] [--body-json JSON] [--header KEY=VALUE]... [--headers-json JSON] [--auth-json JSON] [--default-override KEY=VALUE]... [--default-overrides-json JSON] [--inputs-json JSON] [--exports-json JSON] [--signed] [--trace-endpoint URL] [--expected-status CODE]... [--response-contains TEXT]... [--response-not-contains TEXT]... [--json]	surface=default	reason=case maintenance: maintain Store-native execution configuration with optimistic concurrency
