...
func TestConfigEditRequiresEditor(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(configHome, "store-config.json")); !os.IsNotExist(err) {
		t.Fatalf("config edit without EDITOR should not create config file, stat err = %v", err)
	}
}
...
func TestLogsTailAvoidsWholeFileRead(t *testing.T) {
	raw, err := os.ReadFile("operator_commands.go")
	if err != nil {
		t.Fatalf("read operator commands source: %v", err)
	}
	if strings.Contains(string(raw), "os.ReadFile(path)") {
		t.Fatalf("logs tail should stream from the end instead of reading the full log file")
	}
}
...
func TestConfigEditOpensMalformedExistingConfig(t *testing.T) {
	configHome := t.TempDir()
	configPath := filepath.Join(configHome, "store-config.json")
	writeFile(t, configPath, "{broken json\n")
	dir := t.TempDir()
	editorPath := filepath.Join(dir, "editor")
	callsPath := filepath.Join(dir, "editor-calls.txt")
	writeFile(t, editorPath, "#!/bin/sh\nprintf '%s\\n' \"$1\" > \"$EDITOR_CALLS\"\n")
	...
	if !strings.Contains(string(raw), configPath) {
		t.Fatalf("editor should be called with malformed config path, got %q", raw)
	}
	after, err := os.ReadFile(configPath)
	if err != nil {
		t.Fatalf("read malformed config after edit: %v", err)
	}
	if string(after) != "{broken json\n" {
		t.Fatalf("config edit should not rewrite malformed config before editor, got %q", after)
	}
}
