...
func renderMapAtlasHTML(document mapAtlasDocument) (string, error) {
	...
	b.WriteString(`</span></div></header><section class="toolbar"><button id="back-node" type="button">Back</button><label>Search <input id="case-search" type="search" placeholder="case, node, template, workflow"></label><label>Workflow <select id="workflow-filter"><option value="">All workflows</option></select></label><button id="focus-node" type="button">Focus</button><button id="fit-selected" type="button">Fit</button><button id="path-finder-open" type="button">Path Finder</button><button id="reset-view" type="button">Reset</button></section><main><section class="graph-shell"><svg id="edge-layer" aria-hidden="true"></svg><div id="node-layer"></div><svg id="map-atlas-minimap" aria-label="Map minimap"></svg></section><aside><div id="node-history"></div><div id="details"></div></aside></main><div id="path-finder" class="modal hidden"><div class="modal-card"><div class="modal-head"><h2>Path Finder</h2><button id="path-finder-close" type="button">Close</button></div><div class="path-form"><label>From <select id="path-from"></select></label><label>To <select id="path-to"></select></label><button id="path-find-run" type="button">Find Path</button></div><div id="path-finder-result"></div></div></div></div><script>`)
...
function renderDetails(){
  ...
  if(atlasData.plan){html+='<div class="section"><h3>Map run plan</h3><div class="kv"><div>plan</div><div>'+esc(atlasData.plan.planId)+'</div><div>status</div><div>'+esc(atlasData.plan.status)+'</div><div>scope</div><div>'+esc(atlasData.plan.scope||"")+'</div></div></div>'}
  ...
  html+='<div class="section"><h3>Run tasks</h3><div class="list">'+(runTasks.map(taskRow).join("")||"<span class='sub'>No saved run task is linked to this node.</span>")+'</div></div>';
...
function renderPathFinderOptions(){
  const opts='<option value="">Select a case...</option>'+atlasData.nodes.map(function(n){return '<option value="'+esc(n.id)+'">'+esc(n.displayName)+' ('+esc(n.caseId||n.id)+')</option>'}).join("");
