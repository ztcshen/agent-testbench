...
func runTemplatePackage(args []string) error {
	...
	switch args[0] {
	case "init":
		return runTemplatePackageInit(args[1:])
	case "install":
		return runTemplatePackageInstall(args[1:])
	case "pack":
		return runTemplatePackagePack(args[1:])
	case "list":
		return runTemplatePackageList(args[1:])
	case "inspect":
	...
	case "import":
		return runTemplatePackageImport(context.Background(), args[1:])
	case "verify":
		return runTemplatePackageVerify(context.Background(), args[1:])
	default:
		return fmt.Errorf("unknown template-package command: %s", args[0])
	}
}

func runTemplatePackageInit(args []string) error {
	flags := flag.NewFlagSet("template-package init", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	outputPath := flags.String("output", "", "External template package output path")
	profileID := flags.String("id", "local", "Template package id")
	displayName := flags.String("display-name", "Local Template Package", "Template package display name")
	...
	fmt.Printf("Initialized template package: %s\n", report.ID)
	fmt.Printf("Path: %s\n", report.Path)
...
func initProfileBundle(outputPath string, profileID string, displayName string, force bool) (profileInitReport, error) {
	...
	manifest := profile.Bundle{
		...
		Description:      "External template package generated by AgentTestBench.",
	...
	if _, err := os.Stat(readmePath); errors.Is(err, os.ErrNotExist) || force {
		body := "# External Template Package\n\nPublish this package into the selected SQL Store before serving it through AgentTestBench:\n\n```sh\nagent-testbench store use local-personal\nagent-testbench template-package import --from . --store local-personal\nagent-testbench serve --store local-personal\n```\n"
...
func profileInitRequest(outputPath string, profileID string, displayName string) (profileInitOptions, error) {
	...
	case isCoreProfilesPath(options.outputPath):
		return profileInitOptions{}, errors.New("template packages must be initialized outside this core repository")
	default:
		return options, nil
	}
}

func runTemplatePackageExport(ctx context.Context, args []string) error {
	flags := flag.NewFlagSet("template-package export", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	storeRef := flags.String("store", "", "Named Store config or Store DSN")
	storeURL := flags.String("store-url", "", legacyStoreURLFlagHelp)
	outputPath := flags.String("output", "", "Template package output path")
	force := flags.Bool("force", false, "Overwrite generated template package manifest when it already exists")
	...
	fmt.Printf("Exported template package: %s\n", report.ProfileID)
	fmt.Printf("Path: %s\n", report.Output)
...
func exportProfileCatalogFromStore(ctx context.Context, s store.Store, outputPath string, force bool) (profileExportReport, error) {
	...
	if isCoreProfilesPath(outputPath) {
		return profileExportReport{}, errors.New("template packages must be exported outside this core repository")
	}
	...
	if err := os.WriteFile(manifestPath, append(raw, '\n'), 0o644); err != nil {
		return profileExportReport{}, fmt.Errorf("write template package manifest %s: %w", manifestPath, err)
	}
	if _, err := profile.Load(outputPath); err != nil {
		return profileExportReport{}, fmt.Errorf("exported template package is invalid: %w", err)
	}
...
func runTemplatePackageInstall(args []string) error {
	flags := flag.NewFlagSet("template-package install", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	from := flags.String("from", "", "External template package path")
	profileHome := flags.String("profile-home", "", "Installed template package home")
	force := flags.Bool("force", false, "Replace an already installed template package")
	jsonOutput := flags.Bool("json", false, "Emit a machine-readable JSON report")
	if err := flags.Parse(args); err != nil {
		return err
	}
	...
	fmt.Printf("Installed template package: %s\n", report.ID)
	fmt.Printf("Path: %s\n", report.TargetPath)
	fmt.Printf("Digest: %s\n", report.BundleDigest)
	return nil
}

func runTemplatePackagePack(args []string) error {
	flags := flag.NewFlagSet("template-package pack", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	profileRef := flags.String("profile", "", "Template package path or installed template package id")
	...
	fmt.Printf("Packed template package: %s\n", report.ID)
...
func runTemplatePackageList(args []string) error {
	flags := flag.NewFlagSet("template-package list", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
	profileHome := flags.String("profile-home", "", "Installed template package home")
	jsonOutput := flags.Bool("json", false, "Emit a machine-readable JSON report")
	if err := flags.Parse(args); err != nil {
	...
	fmt.Printf("Template Package Home: %s\n", report.ProfileHome)
	if len(report.Profiles) == 0 {
		fmt.Println("Template Packages: 0")
		return nil
	}
...
func runTemplatePackageInspect(args []string) error {
	flags := flag.NewFlagSet("template-package inspect", flag.ContinueOnError)
	flags.SetOutput(os.Stderr)
