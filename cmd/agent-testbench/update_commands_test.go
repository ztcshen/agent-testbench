...
func TestUpdateReleaseCheckPeelsAnnotatedTag(t *testing.T) {
	remoteRepo := createBareGitRepoWithFiles(t, "main", map[string]string{
		"cmd/agent-testbench/main.go": "package main\nfunc main() {}\n",
		"go.mod":                      "module update-fixture\n",
	})
	checkout := cloneUpdateFixture(t, remoteRepo)
	localHead := strings.TrimSpace(runGit(t, checkout, "rev-parse", "HEAD"))
	tagAnnotatedUpdateFixture(t, remoteRepo, "main", "v1.0.0")

	out := runCLI(t, "update", "--repo", checkout, "--release", "v1.0.0", "--check", "--json")
	var report struct {
		OK              bool   `json:"ok"`
		CheckOnly       bool   `json:"checkOnly"`
		UpdateAvailable bool   `json:"updateAvailable"`
	...
	if err := json.Unmarshal([]byte(out), &report); err != nil {
		t.Fatalf("decode annotated tag update report: %v\n%s", err, out)
	}
	if !report.OK || !report.CheckOnly || report.UpdateAvailable || report.RemoteRevision != localHead {
		t.Fatalf("annotated tag update report = %#v, local head %s", report, localHead)
	}
}
...
func tagAnnotatedUpdateFixture(t *testing.T, remoteRepo string, branch string, tag string) {
	t.Helper()
	work := filepath.Join(t.TempDir(), "remote-work")
	runGit(t, "", "clone", "--branch", branch, remoteRepo, work)
	runGit(t, work, "-c", "user.name=Open Test", "-c", "user.email=open-test@example.com", "tag", "-a", tag, "-m", tag)
	runGit(t, work, "push", "origin", tag)
}
