...
| 配置修复计划 | 把 profile audit 问题转换成稳定修复行动项，让 agent 和配置作者知道下一步该改什么。 |
| Agent 友好的目标发现 | agent 先调用 `interface-node discover`、`workflow discover` 或 `case discover`，再用返回的精确 ID 执行报告。 |
| 用例维护目录 | API 用例可维护说明、标签、优先级、owner、状态、可运行文件、执行配置、就绪问题、最新运行状态、稳定性、可解释优先级排序、单次查询集合摘要、维护质量检查、质量修复计划和可执行计划，便于审阅、分派和集合执行；agent 可以把用例草稿应用到外部 profile bundle。 |
...
| 稳定性分析 | 分析最近用例运行历史，识别在通过和失败之间反复切换的已维护用例。 |
| 影响面与优先级计划 | 把变更路径、接口线索、工作流线索、标签、最新状态、稳定性、优先级元数据或用例文本转换成可解释的用例选择、排序后的批量请求、同步报告或异步批量运行。 |
...
- 为接口节点生成可审阅的已维护用例草稿，并应用到外部 profile bundle。
- 在发布到本地 Store 前，把 profile audit 失败转换成稳定修复行动项。
- 在交给其他团队依赖之前，审计已维护用例集合的维护质量。
- 把质量缺口转换成稳定的维护行动项，方便 agent 执行或交给团队 owner。
- 回归前先审计已维护集合的最新 Store 覆盖，判断哪些用例需要重跑。
- 在把集合标记为可靠前，识别近期状态反复切换的不稳定用例。
- 把变更线索转换为可解释的用例计划、优先级候选列表和异步批量请求。
- 在真正执行前，一次获取集合覆盖、就绪情况、稳定性和执行建议。
...
./bin/otsandbox.sh case suite quality \
  --profile sample \
  --store-url "$store" \
  --status active \
  --json

./bin/otsandbox.sh case suite quality-plan \
  --profile sample \
  --store-url "$store" \
  --status active \
  --json
