...
- SQLite 是默认的活动 Store，保存当前沙箱状态、运行时事实、工作流目录、执行状态、Evidence 索引和验证结果。
...
| 能力 | 说明 |
| --- | --- |
| 本地优先 Store | 默认 SQLite，保存 schema、运行索引、用例运行、Evidence 索引、耗时、日志、拓扑和后处理任务。 |
