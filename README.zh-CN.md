...
AgentTestBench 是一个面向 Agent 的 API 工作流测试环境，围绕可复用测试场景
...
很多测试平台停留在单个 case 或线性的 workflow。AgentTestBench 增加了
Store-backed **测试场景图谱** 层：map 是由接口节点、workflow 路径、验证用例族和
夹具物化点组成的可复用 DAG。workflow 不再是互相复制的资产，而是图上的一条命名
路径；case 仍然是可执行的 HTTP/MQ/API 调用，或挂在某个节点上的验证用例。

```mermaid
flowchart LR
  interfaceCases["接口 case 节点<br/>单次 HTTP/MQ/API 调用"]
  existingWorkflows["已有 workflow<br/>线性资产"]
  scenarioMap["测试场景图谱<br/>SQL Store 中的共享 DAG"]
  namedPaths["命名路径<br/>smoke、regression、release"]
  validationFamilies["验证用例族<br/>按接口收拢的负向 case"]
  materializations["物化点<br/>夹具和回放点"]
  explainPlan["map explain<br/>planner"]
  runPlan["map run<br/>resume、retry、rerun"]
  gatePlan["map gate<br/>Evidence 和状态门禁"]
  atlasReview["map atlas<br/>交互式评审"]
...
这一层的目标是让测试资产在产品变复杂后仍然可维护：

- 把已有 workflow 导入同一张图，而不是维护很多相似链路；
- 从 Store 查看 workflow 收敛关系、覆盖率、路径和版本；
- 执行前用 `map explain` 查看确定性的运行计划；
- 支持执行全部 workflow、全部 case，或聚焦某个 node/path/case，并支持 resume 和 retry；
- 把大量负向用例按接口节点收拢，避免主链路被验证用例淹没；
- 通过自包含的 Test Scenario Atlas HTML 页面评审图谱和执行计划。

设计借鉴：

- [Understand Anything](https://github.com/Egonex-AI/Understand-Anything)
  影响了 Atlas 评审层：图谱不只是展示复杂度，而是帮助人理解结构。
  AgentTestBench 将这个思想用于测试资产，通过可搜索节点、聚焦详情、关系分组和
  适合评审的图谱导航，帮助人审查 map 是否合理。
- [TiDB](https://pingcap.github.io/tidb-dev-guide/understand-tidb/execution.html)
  影响了执行模型：planner 产出 plan tree，executor 执行该 plan，operator 暴露足够的
  细节让执行过程可理解。AgentTestBench 将这种 planner/executor 拆分思想迁移到测试
  map 上，对应 `map explain`、已保存 plan task、`map run` 和 `map gate`。

这些是设计思想借鉴，不是运行时依赖或源码引入。
...
| 测试场景图谱 | 将多个 workflow 收敛为 Store-backed DAG，解释确定性的 map plan，按聚焦范围或全量范围执行，进行门禁验收，并生成可交互的 Test Scenario Atlas 评审页面。 |
...
日常操作中，干净机器可以先跑 `onboard`，一次性配置命名 Store、可选构建
runtime、安装 shell 入口并做 smoke；`status` 用于只读查看 checkout、runtime
binary 和 active Store，`doctor` 做本地依赖与配置诊断；需要更新到最新 tag
运行时时用 `update --channel release` 或 `update --release latest`；不确定命令
路径时用 `commands --area AREA --filter TEXT` 搜索。`task run`、`task schedule`、
`task watch`、顶层 `watch` 和 `notify test` 可以把可重复 CLI 操作登记成
Store-backed 任务定义、运行历史、日志和文件/webhook 通知。`config show`、
...
- 维护能力：API 用例元数据、可搜索用例目录、请求模板、夹具、依赖、工作流绑定
  和集合覆盖；
- 测试场景图谱：workflow 导入、map 查看、覆盖率、版本、验证用例族、SQL-style
  explain plan、map 执行、门禁检查和交互式 Atlas 评审；
