...
| 用例维护目录 | API 用例可维护说明、标签、优先级、owner、状态、可运行文件和执行配置，便于审阅、分派和集合执行。 |
| API 用例执行 | 执行单个 HTTP 用例或已维护用例集合，渲染请求、检查断言、写 Evidence，并可把结果索引进 Store。 |
| 集合覆盖审计 | 不重跑请求，直接检查已维护用例集合在 Store 中的通过、失败和未运行状态。 |
