...
- SQL Store 是活动 Store，保存当前沙箱状态、运行时事实、工作流目录、
  执行状态、Evidence 索引和验证结果。PostgreSQL 与 MySQL 都是产品 SQL Store
  引擎，团队按自己的运维环境选择。本地和远端 SQL Store 使用同一套日常命令，
...
- **Store 引擎**：PostgreSQL 与 MySQL 是产品 SQL Store 引擎；SQLite 只保留给
  旧迁移、兼容检查和测试。
...
| 能力 | 说明 |
| --- | --- |
| SQL Store-first | named PostgreSQL 或 MySQL Store 保存 schema、运行索引、用例运行、Evidence 索引、耗时、日志、拓扑和后处理任务。 |
...
设置 `OTS_TRACE_GRAPHQL_URL`、`OTS_SMOKE_EXPECTED_STEPS` 和
`OTS_SMOKE_TRACE_IDS`，让配置工作流 smoke 使用真实 trace id。如果最终验收必须
拒绝 synthetic topology evidence，还要设置 `OTSANDBOX_REQUIRE_REAL_SKYWALKING=1`；
该模式要求 `OTS_SMOKE_TRACE_IDS` 覆盖配置工作流的每一个必需 step。未配置
...
- Store 生命周期：named PostgreSQL/MySQL 配置、active Store 切换、按后端输出
