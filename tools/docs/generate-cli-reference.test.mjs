...
test("CLI reference generator renders the current command catalog", async () => {
  ...
  try {
    ...
    assert.match(markdown, /Total commands: `122`/);
    assert.match(markdown, /## Daily Agent Surface/);
    assert.match(markdown, /## Command Areas/);
    assert.match(markdown, /### `map`/);
    assert.match(markdown, /\| `map run` \| daily \| agent \| stable \|/);
    assert.match(markdown, /\| `case inspect` \| daily \| agent \| stable \|/);
