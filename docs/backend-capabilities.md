...
| Suite and interface reports / 集合与接口报告 | Runs exact case ids, cases selected by maintenance metadata, impact signals, or all cases attached to one interface node, then returns JSON plus temporary HTML, JUnit XML, artifact manifests, and compact failure summaries. Failed cases remain part of the report. 按精确用例 ID、维护条件、影响面线索，或某个接口节点下的全部用例执行，返回 JSON、临时 HTML、JUnit XML、产物清单和紧凑失败摘要；失败用例保留在报告内。 | `case suite report`, `case suite report --view impact-report`, `interface-node case report`, `/api/cases/batch-runs`, `/api/case/suite-impact-runs` |
| Workflow run lookup and gates / 工作流运行查询与门禁 | Plans workflow-bound steps through map explain/run surfaces, reads saved map and saved plan details through map inspect views, and gates persisted workflow runs by run status, step status, and Evidence completeness. 通过 map explain/run 规划 workflow-bound 步骤，通过 map inspect 视图读取已保存的 map 与 plan 详情，并基于运行状态、步骤状态和 Evidence 完整度对持久化 workflow run 做门禁。 | `map explain`, `map inspect`, `workflow gate`, `/api/workflow-plan`, `/api/workflow-runs`, `/api/workflow-runs/{id}` |
...
| Tasks / 任务 | `task run`, `task schedule`, `task watch`, `task list`, `task status`, `task logs`, `task stop`, `notify test` |
