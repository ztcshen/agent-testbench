...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, and execution configuration for review, assignment, coverage, and suite execution. 索引用例说明、标签、优先级、owner、状态、可运行文件和执行配置，便于审阅、分派、覆盖检查和集合执行。 | `case discover`, `case suite coverage`, `case suite report`, Store catalog, profile API case metadata |
...
| Suite and interface reports / 集合与接口报告 | Runs cases selected by maintenance metadata or all cases attached to one interface node, then returns JSON plus a temporary HTML report. Failed cases remain part of the report. 按维护条件选择用例，或执行某个接口节点下的全部用例，返回 JSON 和临时 HTML 报告；失败用例保留在报告内。 | `case suite report`, `interface-node case report`, `/api/cases/batch-runs` |
...
| Coverage and timing / 覆盖与耗时 | Shows interface or maintained-suite coverage, missing coverage, latest status, elapsed time, and configured timing budgets where available. 展示接口或已维护集合覆盖、缺口、最新状态、实际耗时和配置的耗时预算。 | `case suite coverage`, `/api/case/suite-coverage`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/timing` |
...
- `POST /api/cases/batch-runs`: start an asynchronous batch for interface
  nodes, one workflow, or a maintained suite selector. 为接口节点、工作流或已维护集合条件启动异步批量执行。
...
| Coverage / 覆盖 | `case suite coverage`, `case incomplete-batches`, `/api/case/suite-coverage`, `/api/interface-node/coverage` |
