...
| Coverage and timing / 覆盖与耗时 | Shows interface or maintained-suite coverage, missing coverage, latest status, elapsed time, and configured timing budgets where available. 展示接口或已维护集合覆盖、缺口、最新状态、实际耗时和配置的耗时预算。 | `case suite coverage`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/timing` |
...
| Coverage / 覆盖 | `case suite coverage`, `case incomplete-batches`, `/api/interface-node/coverage` |
