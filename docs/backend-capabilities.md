...
| --- | --- | --- |
| CLI command catalog and operator diagnostics / 命令目录与操作诊断 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen, plus clean-machine onboard/setup, status, doctor, update, local config, log, shell completion, Store-backed task, watch, and notification commands. Agents and automation can discover command paths, areas, Store awareness, tags, and concrete next actions before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录，并提供干净机器 onboard/setup、status、doctor、update、本地配置、日志、shell completion、Store-backed task/watch/notify 命令；agent 和自动化可在执行前发现命令路径、领域、Store 感知、标签和具体下一步动作。 | `onboard`, `setup`, `status`, `doctor`, `update`, `commands`, `task run`, `task watch`, `notify test`, `config show`, `logs`, `completion`, `help` |
...
| --- | --- |
| General diagnostics / 基础诊断 | `onboard`, `setup`, `status`, `doctor --fix`, `doctor --deep`, `update --channel main|release`, `commands --area`, `config show`, `logs`, `completion` |
