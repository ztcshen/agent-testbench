...
| --- | --- | --- |
| CLI command catalog and operator diagnostics / 命令目录与操作诊断 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen. The default catalog is the daily agent surface; `commands --all` exposes advanced, compatibility, and developer/operator entries. Agents and automation can discover command paths, areas, Store awareness, tags, tier, audience, stability, replacement guidance, and concrete next actions before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录；默认目录是 agent 日常入口，`commands --all` 才展示 advanced、compatibility、developer/operator 入口；agent 和自动化可在执行前发现命令路径、领域、Store 感知、标签、层级、受众、稳定性、替代命令和具体下一步动作。 | `status`, `doctor`, `commands`, `commands --all`, `task run`, `map explain`, `case run`, `environment restore` |
