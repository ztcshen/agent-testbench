...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, and execution configuration for review and assignment. 索引用例说明、标签、优先级、owner、状态、可运行文件和执行配置，便于审阅和分派。 | `case discover`, Store catalog, profile API case metadata |
