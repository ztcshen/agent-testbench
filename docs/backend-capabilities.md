...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, execution configuration, readiness issues, latest run state, stability signals, impact reasons, and executable plans for review, assignment, coverage, impact analysis, and suite execution. CLI reports, Control plane coverage, stability, inspection, planning, impact, impact execution, and batch rerun selectors share one backend rule module. 索引用例说明、标签、优先级、owner、状态、可运行文件、执行配置、就绪问题、最新运行状态、稳定性信号、影响原因和可执行计划，便于审阅、分派、覆盖检查、影响面分析和集合执行；CLI 报告、控制平面覆盖率、稳定性、检查、计划、影响面、影响面执行和批量重跑选择器共用同一套后端规则模块。 | `internal/casesuite`, `case discover`, `case suite stability`, `case suite inspect`, `case suite plan`, `case suite impact`, `case suite impact-report`, `case suite coverage`, `case suite report`, Store catalog, profile API case metadata |
...
| Coverage and timing / 覆盖与耗时 | Shows interface or maintained-suite coverage, missing coverage, latest status, recent stability, elapsed time, readiness gaps, and configured timing budgets where available. 展示接口或已维护集合覆盖、缺口、最新状态、近期稳定性、实际耗时、就绪缺口和配置的耗时预算。 | `case suite coverage`, `case suite stability`, `case suite inspect`, `/api/case/suite-coverage`, `/api/case/suite-stability`, `/api/case/suite-inspection`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/timing` |
...
| Reports / 报告 | `case suite report`, `case suite impact-report`, `interface-node case report`, `workflow report` |
| Coverage / 覆盖 | `case suite coverage`, `case suite stability`, `case suite inspect`, `case suite plan`, `case suite impact`, `case incomplete-batches`, `/api/case/suite-coverage`, `/api/case/suite-stability`, `/api/case/suite-inspection`, `/api/case/suite-plan`, `/api/case/suite-impact`, `/api/case/suite-impact-runs`, `/api/interface-node/coverage` |
