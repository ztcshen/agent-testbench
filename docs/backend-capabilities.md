...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, execution configuration, readiness issues, latest run state, stability signals, impact reasons, priority scores, one-call suite briefs, and executable plans for review, assignment, coverage, prioritization, impact analysis, and suite execution. CLI reports, Control plane coverage, stability, priority, brief, inspection, planning, impact, impact execution, and batch rerun selectors share one backend rule module. 索引用例说明、标签、优先级、owner、状态、可运行文件、执行配置、就绪问题、最新运行状态、稳定性信号、影响原因、优先级评分、单次查询集合摘要和可执行计划，便于审阅、分派、覆盖检查、优先级排序、影响面分析和集合执行；CLI 报告、控制平面覆盖率、稳定性、优先级、摘要、检查、计划、影响面、影响面执行和批量重跑选择器共用同一套后端规则模块。 | `internal/casesuite`, `case discover`, `case suite stability`, `case suite priority`, `case suite brief`, `case suite inspect`, `case suite plan`, `case suite impact`, `case suite impact-report`, `case suite coverage`, `case suite report`, Store catalog, profile API case metadata |
...
| Coverage / 覆盖 | `case suite coverage`, `case suite stability`, `case suite priority`, `case suite brief`, `case suite inspect`, `case suite plan`, `case suite impact`, `case incomplete-batches`, `/api/case/suite-coverage`, `/api/case/suite-stability`, `/api/case/suite-priority`, `/api/case/suite-brief`, `/api/case/suite-inspection`, `/api/case/suite-plan`, `/api/case/suite-impact`, `/api/case/suite-impact-runs`, `/api/interface-node/coverage` |
