...
| --- | --- | --- |
| CLI command catalog / 命令目录 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen, so agents and automation can discover command paths, areas, Store awareness, and tags before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录，方便 agent 和自动化在执行前发现命令路径、领域、Store 感知和标签。 | `commands`, `help` |
| Feature research / Feature 研究 | Consumes the external GitHub Feature Radar index, checks index freshness before use, audits reference and project-index policy locally, ranks fuzzy feature queries through the generated token index with matched-token diagnostics, optional live-reference checks, and copyable follow-up commands, lists and filters feature records, expands feature-backed project references from the maintained project index, gates the full feature index and individual CLI design slices on enough recent 3K+ star open-source references, checks selected references against live GitHub repository metadata for policy failures and local-index drift, explains feature-to-reference matrices with project-index metadata, builds refresh plans from stale/audit/coverage/live-drift signals, ranks roadmap candidates by reference coverage and command availability, turns them into a stateless implementation backlog, and can produce a query-backed research brief, command-backed research map, scope-backed release-check map, executable radar sync workflow, or one-shot research plan with catalog-verified next commands and verification commands without bundling crawler data into core. `research compare` ranks multiple fuzzy feature candidates with reference, command, star, and optional live-check signals before a slice is chosen. `research command` starts from a concrete AgentTestBench command path, confirms it exists in the current catalog, maps it back to radar feature records and references, and returns live-aware `gate`, `plan`, `roadmap`, and `compare` follow-up commands. `research scope` starts from touched files or directories, maps that scope to radar feature candidates, and emits the matching scoped `npm run release-check -- --scope ...` command beside research follow-ups. `research references` resolves a feature query first, preserves ranked top matches, then fills out the maintained project list from `projectIndex` metadata. `research live-check` revalidates references and can mark policy-passing but drifted entries as `refresh-needed` when live stars or pushed_at move beyond configured thresholds. `research refresh-plan --live-check` adds those policy/drift signals to the maintenance plan so a locally fresh index can still request refresh. `research roadmap --live-check` verifies candidate references before final ordering so stale candidates do not lead the next CLI slice. `research backlog --live-check` turns that live-checked roadmap into a task queue with stale tasks marked before implementation starts. `research brief` selects the best feature candidate from a fuzzy query, then returns freshness, audit, reference, optional live-reference, and optional command-path gates plus copyable `search`, `matrix`, `gate`, and `plan` commands. `research plan --live-check` carries that live GitHub policy/drift evidence into the implementation plan itself. `research sync` plans or executes the external radar project's test, refresh, status, audit, coverage, and index commands, with `--seed-only` for fast curated-reference refreshes and `--strict-search` for CI; `--live-check` adds a post-sync GitHub policy/drift gate and `liveRefreshPlan` so the maintained feature/project list can be refreshed from AgentTestBench without moving crawler code into core. `research gate` adds a non-zero pre-implementation guard that combines freshness, audit, feature reference coverage, and optional required command-path checks for the CLI slice being changed. 读取外部 GitHub Feature Radar 索引，在使用前检查索引新鲜度，在本地审计引用和项目索引策略，通过生成好的 token index 排序模糊 feature 查询并返回 matched-token 诊断、可选实时 reference 校验和可复制后续命令，列出和过滤 feature 记录，从维护好的项目索引中按 feature 展开参考项目清单，用足够的近期活跃 3K+ star 开源参考为整体 feature 索引和单个 CLI 设计切片做门禁，使用 GitHub 实时仓库元数据检查参考项目的 policy failure 和本地索引漂移，用项目索引元数据解释 feature 到参考项目的矩阵，并从过期、审计、覆盖和 live-drift 信号生成刷新计划，按引用覆盖和命令可用性排序 roadmap 候选，把候选转成无状态实现 backlog，并可生成包含命令目录校验 next commands 与 verification commands 的 query-backed research brief、从已有命令反查 feature 的研究映射、从触达范围反查 feature 与 scoped release-check 的研究映射、可执行 radar sync workflow 或一次性研究计划，同时不把爬虫数据打包进核心；`research compare` 会在选定切片前按 reference、命令、star 和可选 live-check 信号横向排序多个模糊 feature 候选；`research command` 会从具体 AgentTestBench 命令路径出发，确认命令仍在当前目录中，再反查 radar feature、参考项目和带 live-check 的后续命令；`research scope` 会从本次触达文件/目录出发，或通过 `--changed-since` 从 `git diff --name-only` 自动推导目录 scope，反查 radar feature，并直接给出匹配的 scoped release-check 命令；`research references` 会先解析 feature query，保留 ranked top matches，再用 `projectIndex` 元数据补全维护项目清单；`research live-check` 会复核参考项目，并可在 live stars 或 pushed_at 超过阈值时把仍符合政策但已漂移的条目标成 `refresh-needed`；`research refresh-plan --live-check` 会把这些 policy/drift 信号纳入维护计划，让本地 index 新鲜时也能发现需要刷新；`research roadmap --live-check` 会在最终排序前复核候选引用，避免 stale 候选排在下一轮 CLI 切片前面；`research backlog --live-check` 会把 live-checked roadmap 转成任务队列，并在实现前标记 stale 任务；`research brief` 会从模糊 query 选出最佳 feature 候选，并返回 freshness、audit、reference、可选 live-reference 和可选 command-path 门禁，以及可复制的 `search`、`matrix`、`gate` 与 `plan` 命令；`research plan --live-check` 会把 GitHub 实时 policy/drift 证据直接带入实现计划；`research sync` 会从 AgentTestBench 侧编排外部 radar 项目的 test、refresh、status、audit、coverage 与 index 命令，并支持 `--seed-only` 快速刷新 curated references、`--strict-search` 用于 CI 严格失败；加 `--live-check` 会追加 post-sync GitHub policy/drift 门禁和 `liveRefreshPlan`，保持 feature/project 清单可刷新但不把爬虫搬进核心；`research gate` 为正在修改的 CLI 切片增加 freshness、audit、feature 引用覆盖和可选 command-path 校验的非零前置门禁。 | `research search`, `research compare`, `research command`, `research scope`, `research references`, `research live-check`, `research brief`, `research sync`, `research features`, `research feature`, `research status`, `research audit`, `research coverage`, `research matrix`, `research refresh-plan`, `research roadmap`, `research backlog`, `research gate`, `research plan`, `docs/feature-research.md` |
