...
| --- | --- | --- |
| CLI command catalog and operator diagnostics / 命令目录与操作诊断 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen. The default catalog keeps the common operator path focused, while `commands --all` exposes the full catalog. Agents and automation can discover command paths, areas, Store awareness, semantic tags, lifecycle, replacement guidance, and concrete next actions before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录；默认目录保持常用操作入口聚焦，`commands --all` 展示完整目录；agent 和自动化可在执行前发现命令路径、领域、Store 感知、语义标签、生命周期、替代命令和具体下一步动作。 | `status`, `doctor`, `commands`, `commands --all`, `task run`, `map explain`, `case run`, `environment restore` |
