...
| Case maintenance / 用例维护 | Indexes case description, tags, priority, owner, status, runnable file presence, execution configuration, readiness issues, latest run state, impact reasons, and executable plans for review, assignment, coverage, impact analysis, and suite execution. CLI reports, Control plane coverage, inspection, planning, impact, and batch rerun selectors share one backend rule module. 索引用例说明、标签、优先级、owner、状态、可运行文件、执行配置、就绪问题、最新运行状态、影响原因和可执行计划，便于审阅、分派、覆盖检查、影响面分析和集合执行；CLI 报告、控制平面覆盖率、检查、计划、影响面和批量重跑选择器共用同一套后端规则模块。 | `internal/casesuite`, `case discover`, `case suite inspect`, `case suite plan`, `case suite impact`, `case suite coverage`, `case suite report`, Store catalog, profile API case metadata |
...
| Suite and interface reports / 集合与接口报告 | Runs exact case ids, cases selected by maintenance metadata, or all cases attached to one interface node, then returns JSON plus temporary HTML, JUnit XML, artifact manifests, and compact failure summaries. Failed cases remain part of the report. 按精确用例 ID、维护条件选择用例，或执行某个接口节点下的全部用例，返回 JSON、临时 HTML、JUnit XML、产物清单和紧凑失败摘要；失败用例保留在报告内。 | `case suite report`, `interface-node case report`, `/api/cases/batch-runs` |
...
| Evidence lookup / 证据查询 | Reads request, response, assertions, summaries, fixture context, stored topology, persisted logs, batch artifact manifests, and batch failure summaries for a run or case run. 按运行或 case run 查询请求、响应、断言、摘要、前置数据、拓扑、持久化日志、批量产物清单和失败摘要。 | `evidence list`, `/api/case/evidence`, `/api/case-run/evidence`, `/api/cases/batch-runs/{id}/artifacts.json`, `/api/cases/batch-runs/{id}/failures.json` |
...
| Coverage and timing / 覆盖与耗时 | Shows interface or maintained-suite coverage, missing coverage, latest status, elapsed time, readiness gaps, and configured timing budgets where available. 展示接口或已维护集合覆盖、缺口、最新状态、实际耗时、就绪缺口和配置的耗时预算。 | `case suite coverage`, `case suite inspect`, `/api/case/suite-coverage`, `/api/case/suite-inspection`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/timing` |
...
| Discovery / 目标发现 | `interface-node discover`, `workflow discover`, `case discover` |
| Reports / 报告 | `case suite report`, `interface-node case report`, `workflow report` |
| Coverage / 覆盖 | `case suite coverage`, `case suite inspect`, `case suite plan`, `case suite impact`, `case incomplete-batches`, `/api/case/suite-coverage`, `/api/case/suite-inspection`, `/api/case/suite-plan`, `/api/case/suite-impact`, `/api/interface-node/coverage` |
