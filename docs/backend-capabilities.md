...
Open Test Sandbox backend is a CLI-first, PostgreSQL Store-first control plane
...
Open Test Sandbox 的后端是一套 CLI-first、PostgreSQL Store-first 的集成测试
...
| --- | --- | --- |
| Store / 本地事实库 | Holds schema state, import bundle indexes, run records, case run records, Evidence indexes, baseline gates, timing summaries, runtime logs, topology records, and post-process tasks. 保存 schema、配置索引、运行记录、用例运行、证据索引、基线、耗时、运行日志、拓扑记录和后处理任务。 | `internal/store`, `internal/store/open`, `internal/store/postgres`, `internal/store/sqlstore`, `store status`, `store upgrade`, `/api/baseline/gate` |
| Environment Catalog / 环境目录 | Registers, discovers, inspects, bootstraps, verifies, and publishes verified test environments through the active Store. Verified discovery requires a passed acceptance workflow plus complete Evidence and real SkyWalking topology. 通过 active Store 登记、发现、查看、初始化、验证并发布已验收环境；进入 verified 发现列表必须有通过的验收工作流、完整 Evidence 和真实 SkyWalking 拓扑。 | `environment register`, `environment discover`, `environment inspect`, `environment bootstrap`, `environment verify`, `environment publish-verified`, `/api/environments/*` |
| Import Bundle lifecycle / 配置包生命周期 | Creates, installs, packs, audits, turns audit issues into repair actions, verifies, imports, and publishes external import bundle bundles. Runtime files and local databases are filtered out. OpenAPI JSON and static HTTP capture import planning can derive reviewable draft services, interface nodes, request templates, API cases, and runnable case files without writing directly to an existing import bundle. OpenAPI and HTTP capture import planning are also available through the control-plane API. OpenAPI generation planning can derive reviewable draft negative cases from request schemas through CLI and API. 创建、安装、打包、审计、把审计问题转换成修复行动、验证、导入和发布外部配置包，并过滤运行产物和本地数据库；OpenAPI JSON 与静态 HTTP capture 导入计划可派生可审查的 draft 服务、接口节点、请求模板、API 用例和可运行用例文件，不直接写入已有 import bundle；OpenAPI 与 HTTP capture import planning 也可通过控制平面 API 使用；OpenAPI 生成计划可通过 CLI 和 API 从请求 schema 派生可审查的 draft 负向用例。 | `template-package init`, `template-package install`, `template-package pack`, `template-package audit`, `template-package audit-plan`, `template-package import-plan openapi`, `template-package import-plan http-capture`, `template-package generation-plan openapi`, `template-package verify`, `internal/profileimport/openapi`, `internal/profileimport/httpcapture`, `internal/profilegenerate/openapi`, `/api/template-packages/*` |
...
1. A import bundle bundle is authored outside the core repository.
   配置包在核心仓库外维护。
2. The import bundle is installed, audited, and published into the local Store.
...
The Store is an index and runtime fact database. Import Bundle files and Evidence
...
### Import Bundle and Catalog / 配置与目录

- `GET /api/import bundle`: active import bundle summary. 当前配置摘要。
- `GET /api/import bundle/assets`: active import bundle asset summary. 当前配置资产摘要。
- `GET /api/import bundle/installed`: installed import bundle bundles under the import bundle
  home. 配置 home 下已经安装的配置包。
- `POST /api/import bundle/install`: install a import bundle directory or archive. 安装配置目录或归档包。
- `POST /api/import bundle/import`: publish a import bundle into Store/read-models. 发布配置到 Store/read-model。
- `POST /api/import bundle/audit-plan`: convert audit issues into stable repair
  actions. 将审计问题转换成稳定修复行动项。
- `POST /api/import bundle/verify`: audit, publish, and run configured acceptance
...
- `GET /api/dashboard`: dashboard summary derived from import bundle and Store.
  由配置和 Store 派生的看板摘要。
...
- `GET /api/environments/{environmentId}`: inspect runtime facts, workflow
  coverage, Evidence health, and verification status. 查看运行事实、工作流覆盖、
  Evidence 健康度和验收状态。
- `POST /api/environments/{environmentId}/bootstrap`: prepare Store rows and
  local runtime facts before the first run. 首次运行前准备 Store 记录和本地运行事实。
- `POST /api/environments/{environmentId}/verify`: run the configured
  acceptance workflow and persist verification results, Evidence indexes, and
  real SkyWalking topology. 执行配置的验收工作流，并持久化验收结果、Evidence 索引和真实
  SkyWalking 拓扑。
- `POST /api/environments/{environmentId}/publish-verified`: publish into the
  verified discovery list only after the workflow passed and Evidence plus real
  SkyWalking topology are complete. 仅在工作流通过且 Evidence 与真实 SkyWalking 拓扑完整后
  发布到 verified 发现列表。
...
- `POST /api/cases/run`: run one case from import bundle configuration. 运行配置中的一个用例。
...
- `GET /api/case/runs`: case run list with report-facing failure categories
  and import bundle-owned ordered `failureCategories` rules for triage. 用例运行列表，
  包含面向报告的失败分类和 import bundle 维护的有序分类规则。
...
| Environment Catalog / 环境目录 | `environment register`, `environment discover`, `environment inspect`, `environment bootstrap`, `environment verify`, `environment publish-verified`, `/api/environments/*` |
| Import Bundle / 配置包 | `import bundle init`, `import bundle install`, `import bundle pack`, `import bundle list`, `import bundle inspect`, `import bundle audit`, `import bundle audit-plan`, `import bundle import-plan openapi`, `import bundle import-plan http-capture`, `import bundle generation-plan openapi`, `import bundle verify`, `import bundle import`, `/api/import bundle/audit-plan` |
...
| Acceptance / 验收 | `baseline get`, `baseline set`, `/api/baseline/gate`, `case incomplete-batches`, `workflow audit` |
| Service / 服务 | `serve` |
...
- temporary import bundle-home directories / 临时 import bundle-home 目录。
...
- PostgreSQL is the active product Store for personal and team workflows. PostgreSQL 是个人与团队工作流的当前产品主路径 Store。
