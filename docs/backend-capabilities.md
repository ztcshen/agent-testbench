...
| Workflow run lookup and gates / 工作流运行查询与门禁 | Plans workflow-bound steps through map explain/run surfaces, reads saved map plan details, and gates persisted workflow runs by run status, step status, and Evidence completeness. 通过 map explain/run 规划 workflow-bound 步骤，读取已保存 map plan 详情，并基于运行状态、步骤状态和 Evidence 完整度对持久化 workflow run 做门禁。 | `map explain`, `map plans`, `map plan inspect`, `workflow gate`, `/api/workflow-plan`, `/api/workflow-runs`, `/api/workflow-runs/{id}` |
