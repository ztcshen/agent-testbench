...
Open Test Sandbox backend is a local-first control plane for API-operated
...
| --- | --- | --- |
| Store / 本地事实库 | Holds schema state, import bundle indexes, run records, case run records, Evidence indexes, baseline gates, timing summaries, runtime logs, topology records, and post-process tasks. 保存 schema、配置索引、运行记录、用例运行、证据索引、基线、耗时、运行日志、拓扑记录和后处理任务。 | `internal/store`, `internal/store/sqlite`, `store status`, `store upgrade` |
| Import Bundle lifecycle / 配置包生命周期 | Creates, installs, packs, audits, turns audit issues into repair actions, verifies, imports, and publishes external import bundle bundles. Runtime files and local databases are filtered out. OpenAPI JSON and static HTTP capture import planning can derive reviewable draft services, interface nodes, request templates, API cases, and runnable case files without writing directly to an existing import bundle. OpenAPI generation planning can derive reviewable draft negative cases from request schemas. 创建、安装、打包、审计、把审计问题转换成修复行动、验证、导入和发布外部配置包，并过滤运行产物和本地数据库；OpenAPI JSON 与静态 HTTP capture 导入计划可派生可审查的 draft 服务、接口节点、请求模板、API 用例和可运行用例文件，不直接写入已有 import bundle；OpenAPI 生成计划可从请求 schema 派生可审查的 draft 负向用例。 | `import bundle init`, `import bundle install`, `import bundle pack`, `import bundle audit`, `import bundle audit-plan`, `import bundle import-plan openapi`, `import bundle import-plan http-capture`, `import bundle generation-plan openapi`, `import bundle verify`, `internal/import bundleimport/openapi`, `internal/import bundleimport/httpcapture`, `internal/import bundlegenerate/openapi`, `/api/import bundle/*` |
| Catalog read-model / 目录视图 | Converts import bundle assets into service, workflow, interface node, case, template, fixture, dependency, and binding views for the UI and agent commands. 将配置资产转换为服务、工作流、接口节点、用例、模板、夹具、依赖和绑定视图。 | `/api/catalog`, `/api/dashboard`, `/api/import bundle/catalog-index` |
...
| Evidence lookup / 证据查询 | Reads request, response, assertions, summaries, fixture context, stored topology, persisted logs, batch artifact manifests, and batch failure summaries for a run or case run. Evidence records include attachment category, visibility, and labels metadata; failure summaries include local failure categories for triage, with optional import bundle-owned ordered category rules. Report previews redact common sensitive JSON keys and URL query parameters. 按运行或 case run 查询请求、响应、断言、摘要、前置数据、拓扑、持久化日志、批量产物清单和失败摘要；Evidence 记录包含附件分类、可见性和标签元数据；失败摘要包含本地失败分类，并支持 import bundle 维护的有序分类规则；报告预览会脱敏常见敏感 JSON 键和 URL 查询参数。 | `internal/redaction`, `evidence list`, `/api/case/evidence`, `/api/case-run/evidence`, `/api/cases/batch-runs/{id}/artifacts.json`, `/api/cases/batch-runs/{id}/failures.json`, `failureCategories` |
...
3. Catalog/read-model APIs expose the import bundle to the workbench and CLI.
...
- `GET /api/import bundle/catalog-index`: latest Store catalog index. 最新 Store catalog 索引。
...
| Execution / 执行 | `executor plan`, `case run`, `template render` |
