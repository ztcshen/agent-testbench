...
| --- | --- | --- |
| CLI command catalog and operator diagnostics / 命令目录与操作诊断 | Exposes a searchable machine-readable command catalog derived from the same Usage source as the help screen. The default catalog keeps the common operator path focused, `commands --all` exposes the public full catalog, and `commands --all --internal` adds maintenance diagnostics for runtime/debug work. Agents and automation can discover command paths, areas, Store awareness, semantic tags, lifecycle, replacement guidance, and concrete next actions before executing. 暴露从 help 同源 Usage 解析出的可搜索机器可读命令目录；默认目录保持常用操作入口聚焦，`commands --all` 展示公开完整目录，`commands --all --internal` 额外展示运行时/排障维护命令；agent 和自动化可在执行前发现命令路径、领域、Store 感知、语义标签、生命周期、替代命令和具体下一步动作。 | `status`, `doctor`, `commands`, `commands --all`, `commands --all --internal`, `task run`, `map explain`, `case run`, `environment restore` |
| Store / 本地事实库 | Holds schema state, template package indexes, run records, case run records, Evidence indexes, baseline gates, timing summaries, runtime logs, topology records, post-process tasks, and CLI task definitions/run history. 保存 schema、配置索引、运行记录、用例运行、证据索引、基线、耗时、运行日志、拓扑记录、后处理任务，以及 CLI 任务定义和运行历史。 | `internal/store`, `internal/store/open`, `internal/store/postgres`, `internal/store/mysql`, `internal/store/sqlstore`, `store status`, `store upgrade`, `task list`, `/api/baseline/gate` |
| Environment Catalog / 环境目录 | Registers, discovers, inspects, bootstraps, restores, checks, stops, records verification results, and publishes verified test environments through the active Store. Compose and explicit start-command lifecycles use external status/health observations; reports, events, and persisted lifecycle summaries redact commands and secrets. Verified discovery requires a passed acceptance workflow plus indexed Evidence and complete real SkyWalking topology in the selected Store. 通过 active Store 登记、发现、查看、初始化、恢复、检查、停止、记录验收结果并发布已验收环境；Compose 与显式 start-command 生命周期使用外部状态或健康探测，报告、事件和持久化生命周期摘要会脱敏命令和密钥；进入 verified 发现列表必须在所选 Store 内有通过的验收工作流、已索引 Evidence 和完整真实 SkyWalking 拓扑。 | `environment register`, `environment discover`, `environment inspect`, `environment bootstrap`, `environment restore`, `environment status`, `environment stop`, `environment verify`, `environment publish-verified`, `/api/environments/*` |
| Template Package lifecycle / 模板包生命周期 | Creates, installs, packs, lists, inspects, exports, verifies, imports, and restores optional template package artifacts. Runtime files and local databases are filtered out. Reviewable OpenAPI and HTTP-capture planning remain available through domain/API modules; the CLI publish path uses `template-package import` to publish an already reviewed package into the active Store. 创建、安装、打包、列出、查看、导出、校验、导入和恢复可选模板包资产，并过滤运行产物和本地数据库；OpenAPI 与 HTTP capture 的可审查计划能力保留在 domain/API 模块中，CLI 发布路径使用 `template-package import` 将已审阅包发布到 active Store。 | `template-package init`, `template-package install`, `template-package pack`, `template-package list`, `template-package inspect`, `template-package export`, `template-package catalog list --active`, `template-package catalog restore`, `template-package verify`, `template-package import`, `internal/domain/profileimport/openapi`, `internal/domain/profileimport/httpcapture`, `internal/domain/profilegenerate/openapi`, `/api/template-packages/*` |
...
| Executor planning / 执行器规划 | Validates template package-owned descriptors for existing external tools or scripts such as Playwright, Postman, k6, pytest, Karate, and custom commands. API cases can reference external source files through active executor descriptors without turning the core into a tool-specific DSL. The current surface is dry-run only and does not execute external binaries. 校验 template package 中声明的外部测试工具或脚本描述，例如 Playwright、Postman、k6、pytest、Karate 和自定义命令；API 用例可以通过 active executor 描述引用外部 source 文件，但核心不变成某个工具的 DSL；当前入口只做 dry-run 规划，不执行外部二进制。 | `internal/runner/executor`, `executor plan`, `executors/*.json`, `apiCases[].executorId` |
| Case maintenance / 用例维护 | Indexes case metadata, lifecycle, readiness, coverage, quality, stability, priority, and impact views. Store mutations use optimistic revisions and strong ETags; concurrent writes return a conflict, immutable revisions are inspectable, and rollback creates a new revision. New cases default to `draft`; only validated `active` cases are executable-ready. 索引用例元数据、生命周期、就绪、覆盖、质量、稳定性、优先级和影响面；Store 变更使用乐观修订与强 ETag，并发写返回冲突，历史修订可查看，回滚会创建新修订；新用例默认为 `draft`，只有校验通过的 `active` 用例可执行。 | `case catalog upsert`, `case catalog history`, `case catalog rollback`, `/api/case-catalog`, `/api/case-catalog/history`, `/api/case-catalog/rollback`, Case Catalog workbench |
| API case execution / 用例执行 | Runs one Store or file-backed HTTP case, renders requests, checks assertions, and indexes both success and normalized failure Evidence. Transport, timeout, and materialization failures retain child Run/CaseRun records plus error/assertion/summary artifacts. 执行 Store 或文件型 HTTP 用例，渲染请求、校验断言，并索引成功及规范化失败 Evidence；传输、超时与 materialization 失败均保留子 Run/CaseRun 及错误、断言和摘要产物。 | `case run`, `/api/cases/run`, `/api/test-kit/run` |
| Suite and interface reports / 集合与接口报告 | Runs exact case ids, maintenance selections, impact selections, or all cases on one interface. Async batches persist the parent running state before launch, checkpoint every case, use an owner lease across control-plane instances, and do not replay unknown outcomes. JSON, HTML, JUnit, artifact manifests, and compact failure summaries remain available after restart. 按精确用例 ID、维护/影响面选择或接口节点执行；异步批次在启动前持久化父运行、逐用例 checkpoint，并用 owner lease 协调多控制面实例，不重放未知结果；重启后仍可读取 JSON、HTML、JUnit、产物清单与失败摘要。 | `case suite report`, `case suite report --view impact-report`, `interface-node case report`, `/api/cases/batch-runs`, `/api/case/suite-impact-runs` |
| Workflow run lookup and gates / 工作流运行查询与门禁 | Plans workflow-bound steps through map explain/run, executes independent DAG waves with bounded `--concurrency`, and uses a Store owner/token lease so one saved plan cannot be run twice concurrently. Saved map/plan inspection and gates use persisted task checkpoints, run status, and Evidence completeness. 通过 map explain/run 规划工作流步骤，以受控 `--concurrency` 执行独立 DAG 波次，并用 Store owner/token lease 防止同一保存计划并发重复执行；map/plan 查看与门禁读取持久化任务 checkpoint、运行状态与 Evidence 完整性。 | `map explain`, `map run`, `map inspect`, `workflow gate`, `/api/workflow-plan`, `/api/workflow-runs`, `/api/workflow-runs/{id}` |
| Evidence lookup / 证据查询 | Reads request, response, assertions, summaries, fixture context, stored topology, persisted logs, batch artifact manifests, legacy runtime imports, and batch failure summaries for a run or case run. Evidence list records and case detail attachments expose stable lowerCamel attachment fields for run/case/step relation, media type, size, hash, category, visibility, and parsed labels; failure summaries include local failure categories for triage, with optional template package-owned ordered category rules. `case inspect` collects case run list, Evidence, timing, and diagnosis views under one CLI entry; `evidence inspect` collects run-level Evidence list and post-process task views under one CLI entry; `case diagnose` remains a direct triage shortcut. `case gate` adds a CI-oriented Store gate over case-run status and indexed Evidence completeness. Case Evidence details and report previews redact common sensitive JSON keys, sensitive headers, and URL query parameters. 按运行或 case run 查询请求、响应、断言、摘要、前置数据、拓扑、持久化日志、批量产物清单、旧运行索引导入和失败摘要；Evidence 列表记录和用例详情附件以稳定 lowerCamel 字段暴露运行/用例/step 关系、媒体类型、大小、哈希、分类、可见性和解析后的 labels；失败摘要包含本地失败分类，并支持 template package 维护的有序分类规则；`case inspect` 将 case run 列表、Evidence、耗时和诊断视图收口到一个 CLI 入口；`evidence inspect` 将运行级 Evidence 列表和后处理任务视图收口到一个 CLI 入口；`case diagnose` 保留为直接诊断捷径；`case gate` 基于 case run 状态和 Evidence 完整度提供 CI 门禁；Case Evidence 详情和报告预览会脱敏常见敏感 JSON 键、敏感 header 和 URL query 参数。 | `internal/domain/redaction`, `case inspect`, `case diagnose`, `case gate`, `evidence import`, `evidence inspect`, `/api/evidence/import`, `/api/evidence/list`, `/api/case/evidence`, `/api/case-run/evidence`, `/api/cases/batch-runs/{id}/artifacts.json`, `/api/cases/batch-runs/{id}/failures.json`, `failureCategories` |
| Observability import / 观测导入 | Stores trace topology, exposes replay/log-style Evidence shells, and records post-process task duration/status plus readable `outcome`, `reason`, and `displayStatus` for later review without blocking the main case response. 存储拓扑、暴露回放/日志类 Evidence shell，并记录后处理任务耗时/状态以及可读的 `outcome`、`reason` 和 `displayStatus`，供后续审阅复用，避免阻塞主请求。 | `/api/trace-topology/collect`, `/api/replay/evidence`, `replay evidence`, `/api/post-process-tasks`, `evidence inspect --view tasks`, post-process task records |
| Coverage and timing / 覆盖与耗时 | Shows interface or maintained-suite coverage, missing coverage, latest status, recent stability, priority ranking, elapsed time, readiness gaps, and configured timing budgets where available. 展示接口或已维护集合覆盖、缺口、最新状态、近期稳定性、优先级排序、实际耗时、就绪缺口和配置的耗时预算。 | `interface-node coverage`, `interface-node coverage-gaps`, `case suite report --view coverage`, `case suite report --view stability`, `case suite report --view priority`, `case suite report --view inspect`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/suite-coverage`, `/api/case/suite-stability`, `/api/case/suite-priority`, `/api/case/suite-inspection`, `/api/case/timing` |
...
| Release guardrails / 发布守卫 | Prevents generated state and source-domain terms from entering the core repository, then runs tests and browser smoke checks. Daily PR/slice validation must pass `--scope` or `--scope-file` so release-check only inspects touched directories and selects matching runtime tests; full release sign-off is explicit with `--full`. 防止生成态和来源域词汇进入核心仓库，并运行测试与浏览器冒烟；日常 PR 或单个切片验证必须传 `--scope` / `--scope-file`，只检查本次触达目录并选择匹配测试；正式发布签核需显式传 `--full`。 | `npm run release-check -- --scope PATH`, `npm run release-check -- --full`, `tools/guardrails/check_no_source_domain_core.sh` |
...
4. An agent discovers a target and runs a case, interface-node report, or
   map/workflow run.
   agent 先发现目标，再执行单用例、接口节点报告或 map/workflow 运行。
...
- `POST /api/cases/run`: retired public file-path execution endpoint; returns
  `410 file_path_execution_disabled`. Register the case in the Store catalog
  and use `POST /api/test-kit/run` with `caseId`; local file execution remains
  available through `case run --case PATH`.
  已停用的公开文件路径执行入口；返回 `410 file_path_execution_disabled`。请先将用例注册到
  Store catalog，再通过 `POST /api/test-kit/run` 和 `caseId` 执行；本地文件执行继续使用
  `case run --case PATH`。
- `POST /api/cases/batch-runs`: start an asynchronous batch for interface
  nodes, exact case ids, one workflow, or a maintained suite selector. Target
  URLs and Evidence roots come from the active Store catalog; the public API
  rejects `baseUrl`, `evidenceDir`, and forged `environmentId` fields. Optional
  `timeoutSeconds` must be between 0 and 600.
  为接口节点、精确用例 ID、工作流或已维护集合条件启动异步批量执行；目标 URL 与
  Evidence 根目录来自 active Store catalog，公开 API 拒绝 `baseUrl`、`evidenceDir`
  和伪造的 `environmentId`，可选 `timeoutSeconds` 必须在 0 到 600 之间。
- `POST /api/test-kit/run`: run one Store catalog case. Public requests accept
  `caseId`, `overrides`, and bounded `timeoutSeconds`; workflow/step, target,
  Evidence, run, planner, and trace metadata are available only through typed
  in-process execution. 运行一个 Store catalog 用例；公开请求仅接受 `caseId`、
  `overrides` 和有界 `timeoutSeconds`，工作流步骤、目标、Evidence、运行、规划和
  trace 元数据只允许由进程内 typed 执行入口提供。
...
- `POST /api/case/suite-impact-runs`: start an asynchronous batch from an
  impact plan and return report URLs immediately. Execution targets and
  Evidence roots remain Store-owned and cannot be overridden in this public
  request. 基于影响面计划启动异步批量运行并立即返回报告 URL；执行目标与 Evidence
  根目录仍由 Store 维护，不能通过该公开请求覆盖。
...
| --- | --- |
| General diagnostics / 基础诊断 | `onboard`, `setup`, `status`, `doctor --fix`, `doctor --deep`, `update --channel main|release`, `commands --area`, `commands --all`, `config show`, `logs`, `completion` |
| Tasks / 任务 | `task run`, `task schedule`, `task worker`, `task watch`, `task list`, `task status`, `task logs`, `task stop`, `notify test` |
| Store / Store | `store status`, `store upgrade` |
| Environment Catalog / 环境目录 | `environment register`, `environment configure`, `environment startup-file put`, `environment discover`, `environment inspect`, `environment bootstrap`, `environment restore`, `environment verify`, `environment publish-verified`, `/api/environments/*` |
| Template Package / 模板包 | `template-package init`, `template-package install`, `template-package pack`, `template-package list`, `template-package inspect`, `template-package export`, `template-package catalog list --active`, `template-package catalog list`, `template-package catalog restore`, `template-package verify`, `template-package import`, `/api/template-packages/*` |
...
| Reports / 报告 | `case suite report`, `case suite report --view impact-report`, `case suite report --view quality-report`, `interface-node case report`, `map explain`, `map atlas`, `/api/workflow-plan` |
| Coverage / 覆盖 | `interface-node coverage`, `interface-node coverage-gaps`, `case suite report --view coverage`, `case suite report --view stability`, `case suite report --view priority`, `case suite report --view brief`, `case suite report --view quality`, `case suite report --view quality-plan`, `case suite report --view inspect`, `case suite report --view plan`, `case suite report --view impact`, `case incomplete-batches`, `/api/interface-node/coverage`, `/api/interface-node/coverage-gaps`, `/api/case/suite-coverage`, `/api/case/suite-stability`, `/api/case/suite-priority`, `/api/case/suite-brief`, `/api/case/suite-quality`, `/api/case/suite-quality-plan`, `/api/case/suite-inspection`, `/api/case/suite-plan`, `/api/case/suite-impact`, `/api/case/suite-impact-runs` |
| Execution / 执行 | `executor plan`, `case run`, `template render`, `/api/executor/plan`, `/api/template/render` |
| Evidence / 证据 | `evidence import`, `evidence inspect`, `/api/evidence/import`, `/api/evidence/list`, `/api/post-process-tasks` |
| Acceptance / 验收 | `gate baseline get`, `gate baseline set`, `/api/baseline/gate`, `case incomplete-batches`, `workflow audit`, `environment acceptance start`, `environment acceptance report` |
| Service / 服务 | `serve` |
...
- SQL Store is the active product Store for personal and team workflows:
  SQLite, PostgreSQL, and MySQL are peer Store engines with different operating
  boundaries.
...
- SQL Store one-click environment restore pulls component source code from
  ...
  restore source, and it must not store images, source archives, runtime
  databases, logs, or Evidence payloads in the sandbox Store.
  SQL Store 一键环境恢复从远程 Git 拉组件源码，并在 Docker 启动前只把紧凑的 Store 启动文件或组件配置资产写入恢复工作区；恢复源不应要求单独的本地环境包仓库，也不能把镜像、源码包、运行库、日志或 Evidence payload 存入沙箱 Store。
