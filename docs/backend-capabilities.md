...
| Release guardrails / 发布守卫 | Prevents generated state and source-domain terms from entering the core repository, then runs tests and browser smoke checks. Daily PR/slice validation should pass `--scope` or `--scope-file` so release-check only inspects touched directories and selects matching runtime tests; unscoped runs remain for full release sign-off. 防止生成态和来源域词汇进入核心仓库，并运行测试与浏览器冒烟；日常 PR 或单个切片验证应传 `--scope` / `--scope-file`，只检查本次触达目录并选择匹配测试，未指定范围的全量检查保留给正式发布签核。 | `npm run release-check -- --scope PATH`, `tools/guardrails/check_no_source_domain_core.sh` |
