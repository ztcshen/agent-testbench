...
| Workflow run lookup and gates / 工作流运行查询与门禁 | Plans workflow-bound steps, reads persisted workflow run details, and gates runs by run status, step status, and Evidence completeness. 规划工作流绑定步骤，读取持久化 workflow run 详情，并基于运行状态、步骤状态和 Evidence 完整度对持久化 workflow run 做门禁。 | `workflow plan`, `workflow runs`, `workflow run`, `workflow step`, `workflow latest-step`, `workflow gate`, `/api/workflow-plan`, `/api/workflow-runs`, `/api/workflow-runs/{id}` |
...
| Reports / 报告 | `case suite report`, `case suite report --view impact-report`, `case suite report --view quality-report`, `interface-node case report`, `workflow plan`, `/api/workflow-plan` |
