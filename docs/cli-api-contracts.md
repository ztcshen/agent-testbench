...
| Feature research | `research search`, `research references`, `research brief`, `research sync`, `research features`, `research feature`, `research status`, `research audit`, `research coverage`, `research matrix`, `research refresh-plan`, `research roadmap`, `research backlog`, `research gate`, `research plan` | None | CLI-only design aid. The commands consume the external GitHub Feature Radar index, list/filter available feature records, rank fuzzy feature queries through the generated token index, gate a specific CLI design slice on enough recent 3K+ star open-source references, and return `nextCommands` with catalog-verified `commandPath`, `catalogCommand`, and `available` fields. `research search` now includes diagnostics for indexed/scanned tokens, matched tokens, candidate features, missing query terms, starter tokens, and next commands when a query has no candidates. `research references` keeps project lookup feature-first: it resolves a feature query, preserves ranked `topMatches`, then expands the maintained `projectIndex` entries attached to that feature with language, topics, matched feature ids, stars, pushed date, and evidence reasons. `research brief` turns a fuzzy capability query into a reviewable pre-design runbook with selected feature, freshness/audit/reference/command gates, top references, and copyable `search`, `matrix`, `gate`, and `plan` commands. `research sync` plans or executes the external radar maintenance chain (`npm test`, `refresh`, `status`, `audit`, `coverage`, `index`) from AgentTestBench while keeping the crawler and generated inventory outside core. `research status` checks generated/source timestamp freshness, reports feature, ranked-reference, and project index counts, and emits the external refresh/audit/coverage/index commands when the radar is stale. `research audit` checks every local reference and every maintained `projectIndex` entry for name, URL, star floor, pushed-after recency, project-to-feature attachment, and minimum per-feature references before the CLI trusts the index. `research coverage` verifies that every indexed feature meets the minimum reference count before roadmap or demo work starts. `research matrix` explains a filtered feature set with ranked references, matched project-index features, languages, and evidence reasons so design work stays feature-first while still using the maintained project list. `research refresh-plan` merges freshness, audit, and coverage results into a prioritized maintenance plan with focus features and external radar refresh commands. `research roadmap` ranks feature candidates by reference coverage, command availability, implementation-facing next commands, and star signal, then emits a copyable `planCommand`. `research backlog` turns those roadmap candidates into stateless prioritized tasks with implementation commands, verification commands, references, and acceptance criteria. `research gate` combines freshness, audit, per-feature reference coverage, and optional required command-path checks into a non-zero pre-implementation gate. `research plan` packages the reference gate, ranked references, runnable next commands, and verification commands into text, JSON, or Markdown without adding a crawler or bundled radar data to the core repository. |
