...
- `agent-testbench research scope --changed-since HEAD --include-untracked --live-check`
  derives directory scopes from Git changes, ranks feature candidates, and emits
...
- `agent-testbench research sync --radar-root ./github-feature-radar --strict-search --live-check --execute`
  runs the external radar maintenance chain from AgentTestBench: test, refresh,
  status, audit, coverage, index, and optional live GitHub drift gates, with
  per-step JSON results; use `--seed-only` for fast curated-reference refreshes
  under search rate limits.
...
- `agent-testbench research scope --changed-since HEAD --include-untracked --live-check`
  会从 Git 变更自动推导目录 scope，反推 feature 候选，并直接给出匹配的 scoped
...
- `agent-testbench research sync --radar-root ./github-feature-radar --strict-search --live-check --execute`
  ...
  audit、coverage、index 和可选 GitHub 实时漂移门禁，并返回每一步的 JSON
  结果；遇到搜索限流时可用 `--seed-only` 快速刷新 curated references。
