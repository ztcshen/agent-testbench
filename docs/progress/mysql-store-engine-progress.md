...
- Added env-gated MySQL named active Store coverage for `case suite inspect`
  JSON and text output.
- Shared the existing PostgreSQL inspection scenario through a helper so
  PostgreSQL and MySQL assert identical readiness, latest-status, and suggested
  action behavior against the active Store.
- Reused the unique case suite fixture so shared SQL test databases do not
  contaminate readiness checks.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuiteInspect(ReportsReadinessByMaintenanceFilters|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case suite plan`
  JSON and text output.
- Shared the existing PostgreSQL batch-request scenario through a helper so
  PostgreSQL and MySQL assert identical selected case and batch request
  behavior against the active Store.
- Reused the unique case suite fixture and parameterized request ids by backend
  label so shared SQL test databases do not contaminate planning checks.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuitePlan(BuildsExecutableBatchRequest|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case suite priority`
  JSON and text output.
- Shared the existing PostgreSQL ranked batch request scenario through a
  helper so PostgreSQL and MySQL assert identical selected case, score, and
  batch request behavior against the active Store.
- Reused the unique case suite fixture and parameterized request ids by backend
  label so shared SQL test databases do not contaminate priority checks.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuitePriority(BuildsRankedBatchRequest|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case suite brief`
  JSON and text output.
- Shared the existing PostgreSQL agent-facing summary scenario through a
  helper so PostgreSQL and MySQL assert identical readiness, recommendation,
  and batch request behavior against the active Store.
- Reused the unique case suite fixture and parameterized request ids by backend
  label so shared SQL test databases do not contaminate brief checks.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuiteBrief(SummarizesMaintainedSuiteForAgents|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case suite stability`
  JSON and text output.
- Shared the existing PostgreSQL transition scenario through a helper so
  PostgreSQL and MySQL assert identical latest-status, transition, and
  instability behavior against the active Store.
- Reused the unique case suite fixture so shared SQL test databases do not
  contaminate stability checks.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuiteStability(ReportsTransitions|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- Subagent read-only audit confirmed this path now uses unique case ids and
  keeps the latest-run ordering assertion on `Recent[0]`.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Added env-gated MySQL named active Store coverage for `case suite
  impact-report`.
- Shared the existing PostgreSQL impact-report execution scenario through a
  helper so PostgreSQL and MySQL assert identical impacted case selection,
  execution report counts, case run status, and HTML report output.
- Added a unique interface-node batch report fixture so shared SQL test
  databases do not contaminate impact-report runs.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseSuiteImpactReport(RunsImpactedCases|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `workflow report` when
  a workflow step fails.
- Shared the existing PostgreSQL failure-report scenario through a helper so
  PostgreSQL and MySQL assert identical workflow discovery, report counts,
  failed step evidence handles, and HTML report output.
- Added a unique workflow batch report fixture so shared SQL test databases do
  not contaminate workflow report discovery or run state.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestWorkflowReport(WritesReportWhenStepFails|UsesNamedMySQLActiveStoreWhenStepFails)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case
  incomplete-batches`.
- Shared the existing PostgreSQL incomplete-batches scenario through a helper
  so PostgreSQL and MySQL assert identical text output, JSON output, not-run
  detection, and suggested command behavior against the active Store.
- Kept the local SQLite-only catalog branch as an explicit compatibility
  check inside the shared helper.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseIncompleteBatches(CommandReportsNotRunCases|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `case discover`.
- Shared the existing PostgreSQL maintenance metadata discovery scenario
  through a helper so PostgreSQL and MySQL assert identical tag/status/owner
  filtering, returned metadata, and text-filter behavior against the active
  Store.
- Reused the unique interface-node batch report fixture so shared SQL test
  databases do not contaminate case discovery results.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestCaseDiscover(FiltersByMaintenanceMetadata|UsesNamedMySQLActiveStore)' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `sandbox start`.
- Shared the existing PostgreSQL sandbox start scenario through a helper so
  PostgreSQL and MySQL assert identical active-Store service lookup, startup
  command execution, JSON result shape, and local side-effect verification.
- Kept the test lightweight: it executes a local `printf` startup command and
  does not start Docker containers.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestSandboxStartUsesNamed(PostgreSQL|MySQL)ActiveStore' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Added env-gated MySQL named active Store coverage for `environment restore`
  with fake Docker execution and acceptance workflow recording.
- Shared the existing PostgreSQL active Store restore scenario through a helper
  so PostgreSQL and MySQL assert identical registration, restore execution,
  acceptance API calls, and workflow acceptance status handling.
- Kept the test local and deterministic: it uses fake docker, local compose
  files, and httptest acceptance endpoints.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestEnvironmentRestoreUsesNamed(PostgreSQL|MySQL)ActiveStore' -count=1`
  compiled and passed locally; the env-gated PostgreSQL/MySQL cases skipped
  because local DSNs were not exported in this shell.
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
...
- Extended one-click environment restore policy tests from PostgreSQL-only to
  SQL Store backend parity.
- Added MySQL coverage for SQL Store restore requirements that service
  repositories must be remote Git URLs.
- Added MySQL coverage for SQL Store restore readiness requiring a Store-backed
  component graph before one-click startup proceeds.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestEnvironmentRestoreRequires(RemoteGitSourcesForSQLOneClickEnvironment|ComponentGraphForSQLOneClick)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Extended one-click environment restore startup-file policy tests from
  PostgreSQL-only naming to SQL Store backend parity.
- Added MySQL coverage that accepts Docker startup files generated from Store
  metadata.
- Added MySQL coverage that rejects local package startup files when SQL Store
  restore metadata does not provide generated startup content.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestEnvironmentRestoreSQLStore(UsesStoreGeneratedStartupFiles|RejectsLocalStartupFilesWithoutStoreGeneratedContent)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Extended one-click environment restore compose startup asset tests from
  PostgreSQL-only naming to SQL Store backend parity.
- Added MySQL coverage that rejects compose startup files when referenced
  startup asset paths are missing from Store-generated metadata.
- Added MySQL coverage that accepts compose startup files when the Store
  provides all generated DDL/script startup assets.

Validated:

- `go test -v ./cmd/otsandbox -run 'TestEnvironmentRestoreSQLStore(RejectsMissingComposeStartupAssets|AcceptsStoreGeneratedComposeStartupAssets)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Added MySQL dialect coverage for replacing the Store-backed profile catalog
  snapshot used by catalog/workflow discovery APIs.
- Asserted MySQL `?` bind placeholders and `on duplicate key update` are used
  for `profile_catalogs` upserts.
- Verified the MySQL path can read profile catalog index rows and round-trip the
  stored catalog payload.

Validated:

- `go test -v ./internal/store/sqlstore -run 'TestStoreReplaces(AndReadsProfileCatalogSnapshotThroughDatabaseSQL|ProfileCatalogSnapshotUsesMySQLDialect)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Added MySQL dialect coverage for SQL Store schema status table-existence
  checks, including the MySQL `database()` schema predicate.
- Added MySQL dialect coverage for schema upgrade execution, including MySQL
  `datetime(6)` DDL, `?` bind placeholders, and `on duplicate key update` for
  `schema_versions`.
- Verified latest-version MySQL upgrades do not execute DDL when no migration
  is pending.

Validated:

- `go test -v ./internal/store/sqlstore -run 'TestSchemaStatusAndUpgradeSchemaUse(MySQLMigrations|SharedDatabaseSQLMigrations)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Extended the version-one schema upgrade test from PostgreSQL-only to
  PostgreSQL and MySQL dialect parity.
- Verified MySQL upgrades apply the Environment Catalog DDL with `json`
  columns, not PostgreSQL `jsonb`.
- Kept the existing PostgreSQL environment catalog migration assertion intact.

Validated:

- `go test -v ./internal/store/sqlstore -run '^TestUpgradeSchemaAppliesEnvironmentCatalogToVersionOneDatabase$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Added MySQL dialect coverage for Store-backed Environment Catalog upserts and
  lookups, including verified acceptance metadata.
- Added MySQL coverage for replacing and reading component graph metadata used
  by one-click Docker restore: components, startup dependencies, and
  service-owned DDL assets.
- Extended the fake SQL driver with minimal transaction support so SQL Store
  component graph writes can be verified without a live database.

Validated:

- `go test -v ./internal/store/sqlstore -run '^TestStoreEnvironmentCatalogUsesMySQLDialect$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
...
- Updated `otsandbox store ddl` so `--store NAME_OR_DSN` or the active Store
  can select the DDL backend when `--backend` is omitted.
- Kept backward compatibility for machines with no active Store: bare
  `otsandbox store ddl` still prints PostgreSQL DDL.
- Added named MySQL and active MySQL DDL tests proving MySQL users get `json`
  and `datetime(6)` DDL instead of PostgreSQL `jsonb` or
  `create index if not exists` syntax.

Validated:

- `go test ./cmd/otsandbox -run '^(TestStoreDDLCommandPrintsPostgreSQLSchema|TestStoreDDLCommandPrintsMySQLSchema|TestStoreDDLCommandInfersMySQLBackendFromNamedStore|TestStoreDDLCommandInfersActiveMySQLBackend|TestTopLevelHelpShowsStoreFlagNotLegacyStoreURL)$' -count=1`
- `git diff --check`
- `rg -n -i 'fall''back' . --glob '!node_modules/**'`
- `tools/guardrails/check_store_first_contracts.sh && tools/guardrails/check_no_source_domain_core.sh`
