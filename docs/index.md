...
- [CLI Reference](cli-reference.md): generated command catalog with the daily
  agent surface, advanced entries, replacements, and exact usage.
...
- [Test Plan Maps](test-plan-maps.md): decide when to create a workflow map,
  build it from the Store catalog, search workflow paths, and explain target
  cases. 判断什么时候建 workflow map、如何从 Store catalog 构建、如何搜索
  workflow path，以及如何解释目标 case。
...
- [CLI and API Contracts](cli-api-contracts.md): discovery, single-interface
  reports, map/workflow run lookup, Environment Catalog restore surfaces,
  asynchronous batch reports, topology collection, and failed-case Evidence
  lookup. 目标发现、单接口报告、map/workflow 运行查询、环境恢复接口、异步批量报告、拓扑采集和失败证据查询。
- [CLI Reference](cli-reference.md): generated command catalog for automation
...
- [Core Capabilities, Skills, and Goals](core-capabilities-skills-goals.html):
  ...
  当前工作台能力和方向的可视化总览。
- [Demo Gallery](demo-gallery.md): public-facing visual tour of CLI, service
  scenarios, and Evidence outputs. 面向外部传播的 CLI、服务场景和 Evidence 可视化导览。
