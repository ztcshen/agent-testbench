...
- [Store Backends](store-backends.md): configure PostgreSQL or MySQL Stores for
  personal and team workflows, with SQLite kept only for compatibility.
  配置个人与团队工作流使用的 PostgreSQL 或 MySQL Store，并了解 SQLite 兼容路径。
...
- [Release Checklist](release-checklist.md): strict real SkyWalking sign-off and
  company MySQL final verification path. 真实 SkyWalking 严格验收和公司 MySQL 最终验证路径。
...
testing should use the active PostgreSQL/MySQL SQL Store, Environment Catalog,
CLI/API discovery, and the workbench instead of maintaining a separate package
as the normal operating surface.

模板包是可选的导入、导出、审阅和迁移资产；日常测试应使用 active PostgreSQL/MySQL
